...
create table if not exists public.graph_nodes (
  id text primary key,
  label text not null check (label in ('Person', 'System', 'SOP', 'Rule', 'Step', 'Entity')),
  name text not null,
...
create table if not exists public.graph_edges (
  ...
  edge_type text not null check (edge_type in ('OWNS', 'REQUIRES', 'MODIFIES', 'DEPENDS_ON', 'EXECUTES')),
  properties jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now(),
