...
export function OAuthSetupWizard({
  ...
  const [activeTab, setActiveTab] = useState<"slack" | "github" | "gmail">(
    initialProvider,
  );
  ...
  const getCallbackUrl = () => {
    ...
    if (activeTab === "github")
      return `${appBaseUrl}/api/integrations/github/callback`;
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/api/integrations/platform-config/${activeTab}`,
        {
          method: "POST",
          headers: {
      ...
      if (res.ok) {
        setStatusMsg({
          type: "success",
          text: `Successfully saved ${activeTab.toUpperCase()} OAuth App configuration!`,
        });
        setClientId("");
        setClientSecret("");
        ...
        setStatusMsg({
          type: "error",
          text: errData.error || "Failed to save configuration.",
        });
      }
    } catch {
      setStatusMsg({
        type: "error",
        text: "Network error connecting to backend API.",
      });
    } finally {
      setSaving(false);
    }
      ...
      <div className="glass-panel relative flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl duration-300 ease-out animate-in fade-in zoom-in-95 shadow-[0_20px_60px_rgba(0,0,0,0.15)]">
        <header className="flex items-start justify-between gap-4 border-b border-black/[0.06] p-6">
          <div className="space-y-1">
            ...
            <p className="text-[12.5px] text-[#6e6e73]">
              Set platform-level OAuth credentials for Slack, GitHub, or Gmail
              directly in the UI.
            </p>
          </div>
          <button
          ...
          <div className="rounded-2xl border border-black/[0.08] bg-slate-50/80 p-4 space-y-2.5">
            ...
            <div className="mt-3 flex items-center justify-between gap-2 rounded-xl border border-black/[0.06] bg-white p-2.5">
              <div className="space-y-0.5 overflow-hidden">
                ...
                <p className="truncate text-[11.5px] font-mono text-slate-800">
                  {getCallbackUrl()}
                </p>
              </div>
              <button
        ...
        <footer className="flex items-center justify-between border-t border-black/[0.06] p-4 bg-slate-50/50 text-[11.5px] text-slate-500">
          <p className="flex items-center gap-1">
            <ShieldCheck className="h-3.5 w-3.5 text-emerald-600" /> Client
            Secrets Encrypted via AES-256-GCM at Rest
          </p>
        </footer>
      </div>
    </div>
