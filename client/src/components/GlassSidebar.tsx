...
export function GlassSidebar({ onTeachClick }: { onTeachClick?: () => void }) {
  return (
    <aside className="glass-panel hidden w-64 shrink-0 flex-col rounded-xl p-4 lg:flex">
      <div className="flex items-center gap-3 px-2 py-3">
        <span className="relative flex h-10 w-10 items-center justify-center rounded-lg border border-[#0071e3]/20 bg-gradient-to-b from-[#0071e3] to-[#0284c7] shadow-[0_4px_14px_rgba(0,113,227,0.3)]">
        ...
        {NAV.map((item, i) => {
          ...
          return (
            <button
              ...
              type="button"
              className={[
                "group flex items-center gap-3 rounded-lg px-3.5 py-2.5 text-left text-[13.5px] transition-all duration-300",
                active
      ...
      <div className="mt-auto rounded-lg border border-black/[0.06] bg-white/60 p-3 shadow-sm">
        ...
        <p className="mt-1.5 flex items-center gap-2 text-[13px] font-medium text-[#1d1d1f]">
          <span className="h-2 w-2 rounded-sm bg-[#10b981] shadow-[0_0_8px_rgba(16,185,129,0.6)]" />
          Guardrails active
        </p>
      </div>
    </aside>
