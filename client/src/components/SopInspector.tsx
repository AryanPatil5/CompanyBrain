...
export function SopInspector({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      />
      <div className="glass-panel relative flex max-h-[86vh] w-full max-w-2xl flex-col overflow-hidden rounded-[24px] duration-300 ease-out animate-in fade-in zoom-in-95">
        <header className="flex items-start justify-between gap-4 border-b border-black/[0.06] p-6">
          <div className="space-y-2.5">
            <div className="flex flex-wrap items-center gap-2">
              <span className="rounded-full border border-[#0071e3]/20 bg-[#0071e3]/[0.08] px-2.5 py-1 text-[11px] font-medium text-[#0071e3]">
                {sop.category}
              </span>
          ...
          <button
            ...
            onClick={onClose}
            aria-label="Close"
            className="glass-button flex h-9 w-9 shrink-0 items-center justify-center overflow-hidden rounded-xl text-[#6e6e73] hover:text-[#1d1d1f] active:scale-95"
          >
            <X className="h-4 w-4" />
          </button>
        </header>
        ...
        <div className="flex-1 space-y-3 overflow-y-auto p-6">
          {sop.requiresHumanGate && (
            <div className="rounded-2xl border border-[#f59e0b]/30 bg-[#f59e0b]/[0.08] p-3.5 text-[12.5px] text-[#b45309]">
          ...
          {sop.steps.map((step, i) => (
            ...
            >
              <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-black/[0.08] bg-white font-mono text-[12px] font-semibold text-[#1d1d1f] shadow-sm">
                {i + 1}
              </span>
              <div className="space-y-2 min-w-0">
                ...
                {step.parameters && Object.keys(step.parameters).length > 0 && (
                  <div className="rounded-lg border border-[#0284c7]/15 bg-[#0284c7]/[0.04] px-2.5 py-1.5">
          ...
          {versions.length > 0 && (
            <div className="mt-4">
              <button
                ...
                className="flex w-full items-center gap-2 rounded-2xl border border-black/[0.06] bg-black/[0.02] px-4 py-3 text-left text-[12px] font-medium text-[#6e6e73] transition-colors hover:bg-black/[0.05] active:scale-95"
              >
              ...
              {showVersions && (
                <div className="mt-2 space-y-1.5 pl-2">
                  {versions.map((v) => (
                    <div key={v.id} className="flex items-center gap-3 rounded-xl border border-black/[0.06] bg-white/50 px-3 py-2">
                      <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-black/[0.08] bg-white font-mono text-[10px] font-semibold text-[#1d1d1f]">
                        {v.version_number}
                      </span>
        ...
        <footer className="flex flex-wrap justify-end gap-2 border-t border-black/[0.06] p-5">
          ...
          {sop.isStale && onConfirm && (
            <button
              ...
              className="glass-button flex h-11 items-center gap-2 overflow-hidden rounded-2xl px-6 text-[13px] font-medium text-[#d97706] active:scale-95"
            >
