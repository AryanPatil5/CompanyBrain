        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-[16px] font-semibold text-[#1d1d1f]">
              Multi-Agent State Machine Execution Console
            </h3>
            <p className="text-[12.5px] text-[#6e6e73]">
              Planner DAG Decomposition → Auditor Safety Policy Check → Executor
              Sandbox Runner
            </p>
          </div>
        </div>
        ...
        <div className="space-y-4 rounded-xl border border-slate-200 bg-slate-50/70 p-4">
            ...
            <div className="space-y-1.5 text-[12.5px]">
              ...
              <div className="text-slate-600">
                ...
                <span className="font-semibold text-slate-800">
                  {workflow.audit.risk_level}
                </span>{" "}
                | Approved:{" "}
                <span className="font-semibold">
                  {workflow.audit.approved
                    ? "Yes"
          ...
          {workflow.status === "paused_approval" && workflow.approval_id && (
            <div className="flex items-center justify-between rounded-xl border border-amber-200 bg-amber-50 p-3 text-[12.5px] text-amber-900">
              <span>
                Approval Ticket <strong>#{workflow.approval_id}</strong> created
                in manager queue.
              </span>
              <button
                type="button"
          ...
          {workflow.plan?.steps && (
            <div className="space-y-2">
              <div className="text-[12.5px] font-semibold text-slate-800">
                Planner DAG Execution Steps:
              </div>
              <div className="space-y-2">
                {workflow.plan.steps.map((step) => {
                  const executed = workflow.executed_steps?.find(
                    (e) => e.step_id === step.id,
                  );
                  return (
                    <div
                      key={step.id}
                      className="flex items-center justify-between rounded-lg border border-slate-200 bg-white p-2.5 text-[12px]"
                    >
                      <div>
                      ...
                      <div>
                        {executed ? (
                          <span
                            className={`rounded px-2 py-0.5 text-[10.5px] font-semibold uppercase ${
