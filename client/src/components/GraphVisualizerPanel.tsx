    ...
    <div className="glass-panel rounded-2xl p-5 border border-black/[0.08] space-y-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-[16px] font-semibold text-[#1d1d1f]">
              Apache AGE Enterprise Knowledge Graph
            </h3>
            <p className="text-[12.5px] text-[#6e6e73]">
              PostgreSQL Cypher Vertices (Person, System, SOP, Rule, Step) &
              Relationships
            </p>
          </div>
        </div>
      ...
      {!graphData || graphData.nodes.length === 0 ? (
        <div className="rounded-xl border border-dashed border-slate-300 p-8 text-center text-[13px] text-slate-500 space-y-2">
          ...
          <p>
            No graph nodes found yet. Run an ingestion crawl to extract graph
            entities!
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          <div className="flex items-center gap-4 text-[12px] font-medium text-slate-600">
            <span>
              Vertices (Nodes): <strong>{graphData.nodes.length}</strong>
            </span>
            <span>
              Edges (Relationships): <strong>{graphData.edges.length}</strong>
            </span>
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {graphData.nodes.map((node) => (
              <div
                key={node.id}
                className="rounded-xl border border-slate-200 bg-white/80 p-3.5 space-y-1 text-[12.5px]"
              >
                <div className="flex items-center justify-between">
